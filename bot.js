name: 24-7-Eaglercraft-Macro-Player
...
jobs:
  run-bot:
    ...
    - name: Install Mineflayer and AI Pathfinder
      run: npm install mineflayer mineflayer-pathfinder mineflayer-collectblock

    - name: Inject Clean Survival Script
      run: |
        cat << 'EOF' > bot.js
            ...
            if (tableItem && planks && sticks && !pickaxe) {
                ...
                if (groundBlock) {
                    try {
                        ...
                        setTimeout(async () => {
                            ...
                            if (placedTable) {
                                ...
                                if (pickaxeRecipe) {
                                    ...
                                    setTimeout(() => { bot.collectBlock.collect(placedTable, () => survivalCycleLoop(bot)); }, 2000);
                                    return;
                                }
                            }
                            survivalCycleLoop(bot);
                        ...
                        return;
                    } catch (err) { console.log(`Placement error: ${err.message}`); }
                }
            }
            findAndChopTrees(bot);
        }
        ...
        function findAndChopTrees(bot) {
            ...
            if (targets.length > 0) {
                ...
                if (activeTimeout) clearTimeout(activeTimeout);
                bot.collectBlock.collect(targetBlock, (err) => { setTimeout(() => survivalCycleLoop(bot), 2000); });
            } else {
                ...
                bot.once('goal_reached', () => { setTimeout(() => survivalCycleLoop(bot), 2000); });
            }
        }
